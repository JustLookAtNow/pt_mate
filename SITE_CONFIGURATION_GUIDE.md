...
- `ptm selector`：其实整体上也类似与CSS 选择器，但是更加强大，支持更多的操作，具体有以下不同：
  ...
  - 一些特殊用法：
    ...
    - `next`：表示提取当前元素的下一个元素,注意如果当前标签下有子标签获取的是第一个子标签，如果没有子标签则获取当前标签的下一个兄弟标签。
    - `prev`：表示提取当前元素的上一个元素，注意如果当前标签下有子标签获取的是最后一个子标签，如果没有子标签则获取当前标签的上一个兄弟标签。
    - `nextParsed`：表示提取当前元素的下一个兄弟元素(包括标签和非标签)。
    - `prevParsed`：表示提取当前元素的上一个兄弟元素(包括标签和非标签)。
    - `parent`：表示提取当前元素的父元素。
