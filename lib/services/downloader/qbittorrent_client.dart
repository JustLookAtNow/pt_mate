...
class QbittorrentClient implements DownloaderClient {
  ...
  Future<void> addTask(AddTaskParams params) async {
    ...
    if (params.autoTMM != null) body['autoTMM'] = params.autoTMM.toString();
  ...
  Future<void> pauseTasks(List<String> hashes) async {
    ...
    await _request('POST', apiPath, body: {
      'hashes': hashes.join('|'),
    });
  }
  ...
  Future<void> deleteTasks(List<String> hashes, {bool deleteFiles = false}) async {
    await _request('POST', '/torrents/delete', body: {
      'hashes': hashes.join('|'),
  ...
  DownloadTask _convertToDownloadTask(Map<String, dynamic> torrent) {
    return DownloadTask(
      ...
      tags: torrent['tags']?.toString().split(',').where((tag) => tag.isNotEmpty).toList() ?? [],
